(function () {
    if (EightDigits.sf.prototype.hasOwnProperty("Armani_Kulaklik_Sepet")) {
        return false
    }
    EightDigits.sf.prototype.Armani_Kulaklik_Sepet = function () {
  
        var config = {
            scenarioname: "Armani_Kulaklik_Sepet",
            analytics: "Armani_Kulaklik_Sepet",
            controlgroup: 0
        };
        var itemName = [
            'AR11217',
        ...
        main.factory.prototype = {
                            ...
                            jQuery.ajax(settings).done(function (response) {
                                ...
                                if(uspaPrice>=0){
                                  EightDigits.analytics.sendEvent(config.analytics,'Shown');
                                    var settings = {
                                        "url": "https://www.saatvesaat.com.tr/saat/general/addItem?sku=00GWP-AR002&qty=1",
                                        "method": "POST",
                                      }
                                      jQuery.ajax(settings).done(function (response) {
                                        location.reload();
            ...
            addCss: function () {
                ...
                for (var j = 0; j < cartProducts.length; j++) {
                    if (cartProducts[j].sku === '00GWP-AR002') {
                       
                        jQuery("tr").each(function(){
                            if(jQuery(this).find("h2.product-name").text().trim() == "Emporio Armani Kulalık"){
                                console.log("evet")
    ...
    var _ed_Armani_Kulaklik_Sepet = new EightDigits.sf();
    _ed_Armani_Kulaklik_Sepet.Armani_Kulaklik_Sepet();
  })();
