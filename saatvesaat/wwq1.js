        ...
        main.factory.prototype = {
                            ...
                            jQuery.ajax(settings).done(function (response) {
                                ...
                                if(uspaPrice>=50){
                                  EightDigits.analytics.sendEvent(config.analytics,'Shown');
                                    var settings = {
                                        "url": "https://www.saatvesaat.com.tr/saat/general/addItem?sku=WWQK002&qty=1",
                                        "method": "POST",
                                      }
                                      jQuery.ajax(settings).done(function (response) {
                                        location.reload();
            ...
            addCss: function () {
                ...
                for (var j = 0; j < cartProducts.length; j++) {
                    if (cartProducts[j].sku === 'WWQK002') {
                       
                        jQuery('.cart-price .price:eq('+j+')').css('text-decoration','line-through');
                        jQuery("tr").each(function(){
                            if(jQuery(this).find("h2.product-name").text().trim() == "WWQK002 Akıllı Bileklik Kordonu"){
                    ...
                    uspaItems.map(function(item,i){
                            if(uspaPrice<50){
                                ...
                                jQuery.ajax(settings).done(function (response) {
                                    ...
                                    uspaPrice +=Number(jQuery(response).find("item")[0].getElementsByTagName("g:sale_price")[0].textContent.split(" TRY")[0]) * item.qty
                                    if(uspaItems.length-1 ===i && uspaPrice<50){
