(function () {
    if (EightDigits.sf.prototype.hasOwnProperty("Canta_Hediye_Sepet")) {
        return false
    }
    EightDigits.sf.prototype.Canta_Hediye_Sepet = function () {
  
        var config = {
            scenarioname: "Canta_Hediye_Sepet",
            analytics: "Canta_Hediye_Sepet",
            controlgroup: 0
        };
        var itemName = [
            "MKC1035AN-710",
        ...
        main.factory.prototype = {
                            ...
                            jQuery.ajax(settings).done(function (response) {
                                ...
                                if(uspaPrice>=0){
                                  EightDigits.analytics.sendEvent(config.analytics,'Shown');
                                    var settings = {
                                        "url": "https://www.saatvesaat.com.tr/saat/general/addItem?sku=00GWP-MK1111&qty=1",
                                        "method": "POST",
                                      }
                                      jQuery.ajax(settings).done(function (response) {
                                        location.reload();
            ...
            addCss: function () {
                ...
                for (var j = 0; j < cartProducts.length; j++) {
                    if (cartProducts[j].sku === '00GWP-MK1111') {
                       
                        jQuery("tr").each(function(){
                            if(jQuery(this).find("h2.product-name").text().trim() == "MICHAEL KORS MAKYAJ ÇANTASI"){
                                console.log("evet")
    ...
    var _ed_Canta_Hediye_Sepet = new EightDigits.sf();
    _ed_Canta_Hediye_Sepet.Canta_Hediye_Sepet();
  })();
