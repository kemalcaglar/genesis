  ...
  EightDigits.sf.prototype.Koc_Ailem_Iade = function () {
    ...
    main.factory.prototype = {
      ...
      start: function () {
        ...
        var html = `<span class="smpl-koc-ailem" style="font-weight:900; display:flex; margin-top:10px; margin-left:5px;">72 saate kadar iade garantisinden ücretsiz yararlandınız.</span>`
        ...
        jQuery(".promotion-list .basket-checkbox-top .iCheck-helper").click(function () {

          var this_ = this
          var jsonData = JSON.parse(localStorage.getItem("product"));
          var roomCampaingName = jQuery(this).closest(".basket-checkbox-top").parent().find(".basket-text-top h6").text().trim();
          var basketArray = [];
          jQuery(jsonData).each(function (index, item) {
            if (roomCampaingName == "KoçAilem Kampanyası Şubat") {
              item.roomCampaing = roomCampaingName;

              jQuery(this_).closest(".promotion-list").after(html);
            } else {
              item.roomCampaing = roomCampaingName;
            }
            basketArray.push(item);
          ...
          jQuery(".booking-list.card-list li").each(function () {
            ...
            jQuery(jsonData).each(function (index, item) {
              if (item.hotelName == productName) {
                console.log(item.roomCampaing);
                console.log(jQuery(this_).find(".booking-item .promotion-list .smpl-koc-ailem").length);
                console.log(item.checkBox)
                console.log(jQuery(this_).find(".booking-item .promotion-list"));
                if (item.roomCampaing == "KoçAilem Kampanyası Şubat" && item.checkBox && jQuery(".smpl-koc-ailem").length == 0) {
