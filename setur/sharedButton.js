(function () {
    if (EightDigits.sf.prototype.hasOwnProperty("Share_Icon_Button")) {
        return false
    }
    EightDigits.sf.prototype.Share_Icon_Button = function () {

        var config = {
            scenarioname: "Share_Icon_Button",
            analytics: "Share_Icon_Button",
            controlgroup: 0
        };
...
if(jQuery('.mobile-main-price').length>0){
    jQuery('.mobile-main-price').prepend(shareIcon);
    if(jQuery('.hoteldetail-discounted-price').text().indexOf("İndirim")>-1){
        if(window.innerWidth>374){
            jQuery('.smpl-share-icon-button').css({"float":"right","margin-left":"8px","margin-top":"20px","margin-right":"0px"});
        }
        else if(window.innerWidth>359){
            jQuery('.smpl-share-icon-button').css({"float":"right","margin-left":"6px","margin-top":"20px","margin-right":"0px"});

        }
        else if(window.innerWidth>349){
            jQuery('.smpl-share-icon-button').css({"float":"right","margin-left":"2px","margin-top":"20px","margin-right":"0px"});

        }
        else if(window.innerWidth>333){
            jQuery('.hoteldetail-discounted-price').css({"font-size":"12px","width":"51px"});
            jQuery('.smpl-share-icon-button').css({"float":"right","margin-left":"2px","margin-top":"20px","margin-right":"0px"});

        }
        else if(window.innerWidth>323){
            if(window.innerWidth>329){
                jQuery('.hoteldetail-discounted-price').css({"font-size":"11px","width":"48px","margin-right":"5px"});

            }
            else{
                jQuery('.hoteldetail-discounted-price').css({"font-size":"11px","width":"48px","margin-right":"2px"});
            ...
            jQuery('.hotel-detail-main-price-mobile .mobile-main-price .hoteldetail-discounted-price').css("margin-top","0px")
            jQuery('.smpl-share-icon-button').css({"float":"right","margin-left":"2px","margin-top":"20px","margin-right":"0px"});
    ...
    jQuery('.hoteldetail-header-subinfo').append(shareIcon);
    jQuery('.smpl-share-icon-button').css({"display":"flex","justify-content":"center","float":"right","margin-right":"5px"})
}
var icon_ = {
    ...
    // + '?utm_source=smpl&utm_content=share'
// var message =  jQuery('.hotel-detail-name').text().trim()+' '+encodeURIComponent(window.location.href);
...
<span class='copy-text' style='font-size:11px;display:none;z-index:999999;background-color:white;position:absolute;padding:5px;left:"+(window.innerWidth-94)/2+"px;bottom:83px;border-radius:5px;'>Link Kopyalandı</span>\
<div class='smpl-share-icon'><a  class='smpl-icon-whatsapp' target='_blank' href='whatsapp://send?phone=&text="+encodeURIComponent(jQuery('.hotel-detail-name').text().trim() +" "+ window.location.href)+"'>"+icon_.whatsapp+"</a><span>Whatsapp</span></div>\
...
jQuery('.smpl-share-icon-layer').click(function(){
                ...
                for(var i = 0; i<dataLayer.length;i++){
                    if(!isEcommerce){
                        if(typeof dataLayer[i].ecommerce!=="undefined"){
                            isEcommerce=true;
                            if(dataLayer[i].ecommerce.MainCategory==="Hotel"){
                                isHotel=true;
                            }
    ...
    var _ed_Share_Icon_Button = new EightDigits.sf();
    _ed_Share_Icon_Button.Share_Icon_Button();
})();
