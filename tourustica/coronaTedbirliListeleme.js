(function () {
    if (EightDigits.sf.prototype.hasOwnProperty("Corona_Tedbirli_Badge_New")) {
         return false
    }
    EightDigits.sf.prototype.Corona_Tedbirli_Badge_New = function () {

        var config = {
            scenarioname: "Corona_Tedbirli_Badge_New",
            analytics: "Corona_Tedbirli_Badge_New",
            controlgroup: 10
        };
...
3557
        ...
        main.factory.prototype = {
            ...
            start: function () {
               ...
               if(window.innerWidth>1023){
                jQuery('.hotel-image-wrapper').map(function (index) {
                    ...
                    if ((hotelList.includes(id))) {
                        jQuery(this).append('<img class="smpl-covid-badge" style="position: absolute;width: 80px;height: auto;top: 73%;"  src="https://cdn.8digits.com/log/r/f0cfae00-bc22-4023-b2cb-f840eb613e02.png"></img>');
                ...
                jQuery('.hotel-name a,.hotel-booking-button').click(function(e){
                    if(e.target.parentElement.className==="hotel-name"){
                        if(jQuery(this).parent().parent().parent().parent().find(".smpl-covid-badge").length>0){
                    ...
                    if(e.target.className==="hotel-booking-button"  || e.target.className==="visible-xs" || e.target.className==="hidden-xs"){
                        if(jQuery(this).parent().parent().parent().find(".smpl-covid-badge").length>0){
                            EightDigits.analytics.sendEvent(config.analytics, "Click_Shown_Desktop");
                ...
                jQuery('.hotel-image-wrapper').click(function(e){
                    if(e.target.className==="lazy"){
                        if(jQuery(this).find(".smpl-covid-badge").length>0){
                            EightDigits.analytics.sendEvent(config.analytics, "Click_Shown_Desktop");
                        }
                    }
                })
                ...
                jQuery('.hotel-image-wrapper').map(function (index) {
                    var id = dataLayer[0].HotelIDList[index]
                    if ((hotelList.includes(id))) {
                        if(jQuery(this).parent().find('.hotel-discounts').length>0){
                            jQuery(this).parent().find('.hotel-discounts').prepend('<img class="smpl-covid-badge" style="width: 85px;height: auto;margin-bottom: 3px;margin-left:2px;"  src="https://cdn.8digits.com/log/r/f0cfae00-bc22-4023-b2cb-f840eb613e02.png"></img>');
                        }
                        else{
                            jQuery(this).append('<img class="smpl-covid-badge" style="position: absolute;width: 85px;height: auto;top: 87%;margin-left: 6px;margin-left:2px;"  src="https://cdn.8digits.com/log/r/f0cfae00-bc22-4023-b2cb-f840eb613e02.png"></img>');
                        }
                        if (EightDigits.utils.getCookie('ed-sccontrol' + config.scenarioname) === null) {
                ...
                jQuery('.hotel-image-wrapper').click(function(e){
                    if(e.target.className==="lazy"){
                        if(jQuery(this).parent().find(".smpl-covid-badge").length>0){
                            EightDigits.analytics.sendEvent(config.analytics, "Click_Shown_Mobile");
                        }
                    }
                })
               ...
               console.log(hotelList.find(x=>x===Number(dataLayer[0].HotelID)))
    ...
    var _ed_Corona_Tedbirli_Badge_New = new EightDigits.sf();
    _ed_Corona_Tedbirli_Badge_New.Corona_Tedbirli_Badge_New();
})();
