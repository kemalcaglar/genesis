        ...
        if (EightDigits.utils.getCookie('ed-click_close' + config.scenarioname) === null && data.length >= 2) {
          ...
          if (data.length > 0) {
            ...
            var css = '<style>\
                                     ...
                                     #ed-' + config.scenarioname + '-header{font-weight: bold;font-size: 14px;background-color: #ED3833;color: white;text-align: center;width: 200px;  border-radius: 21px 0px 0px 21px; line-height: 3;}\
          ...
          var x = setInterval(function () {
            ...
            try {
              ...
              if (typeof dataLayer[0].HotelID !== "undefined") { // Otel Detay Sayfası
                if (jQuery('.flex-active img').attr('src').indexOf('https://cdnturint.touristica.com.tr') > -1 || jQuery('.flex-active img').attr('src').indexOf('youtube') > -1 || jQuery(".hotel-detail-top").length > 0) {
                  seenProduct["type"] = "Hotel";
                  seenProduct["sku"] = dataLayer[0].HotelID; // product id
                  seenProduct["productName"] = dataLayer[0].HotelName;
                  seenProduct["newPrice"] = jQuery('.current-price:eq(0)').text().trim(); //discount price
                  seenProduct["location"] = jQuery('.hotel-location').text().split('Harita')[0].trim(); //location
                  if (jQuery('.flex-active img').attr('src').indexOf('youtube') > -1 || jQuery(".hotel-detail-top").length > 0) {
                ...
                if (jQuery('.flex-active-slide:eq(1) img').attr('src').indexOf('https://cdnturint.touristica.com.tr') > -1 || jQuery(".tour-detail-page-v2").length == 1) {
