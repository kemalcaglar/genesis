(function () {
    ...
    function productListWidget(options) {
        ...
        var addedCss = '<style>\
                              .smpl-buttons{display:flex; justify-content:center;padding-top: 6px;padding-bottom: 6px;}\
                              .smpl-edoktor:hover{background-color:white !important; color:black !important;border-color:white !important;}\
                        </style>';
        var addedHtml = '<div class="smpl-buttons">\
                         </div>';
        jQuery('head').append(addedCss);
        jQuery(addedHtml).insertAfter(jQuery('.smpl-productList-title'));

        jQuery('.smpl-buttons a').click(function(){
            var text = jQuery(this).text().trim();

            if(text === "Online Randevu"){
                EightDigits.analytics.sendEvent(config.analytics,'Click_OnlineRandevu');
            }
            else{
                EightDigits.analytics.sendEvent(config.analytics,'Click_Edoktor');
            }
        ...
        jQuery('.smpl-productList-list-item a').on('click', function () {
            var titleClick = jQuery(this).text();
            fireEvent('Click_' + titleClick);
        });
    };
