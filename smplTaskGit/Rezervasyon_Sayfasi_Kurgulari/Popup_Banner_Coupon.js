(function () {
  if (EightDigits.sf.prototype.hasOwnProperty("Popup_Banner_Coupon")) {
    return false;
  }
  EightDigits.sf.prototype.Popup_Banner_Coupon = function () {
    let config = {
      scenarioname: "Popup_Banner_Coupon",
      analytics: "Popup_Banner_Coupon",
    ...
    main.factory.prototype = {
      ...
      control: function () {
        let control = false;
        for (const iterator of dataLayer) {
          if (iterator.ecommerce) {
            if (Object.keys(iterator).includes("ecommerce")) {
              if (iterator.ecommerce.checkout.products.length === 1) {
                const productName = iterator.ecommerce.checkout.products[0].name;
                if (productName === "Butik Mardin Turu" || productName === "Kapadokya Turu Uçaklı") control = true;
              }
            }
          }
        }
  ...
  let _ed_Popup_Banner_Coupon = new EightDigits.sf();
  _ed_Popup_Banner_Coupon.Popup_Banner_Coupon();
})();